...
static int parse_sckey(unsigned char *p)
{
	...
	int i;
	serial_debug("spl->len:		%d\n", spl->len);
	serial_debug("spl->enc_type:	%d\n", spl->enc_type);
	serial_debug("spl->n_bit:		%d\n", spl->n_bit);
	serial_debug("spl->ku_bit:	%d\n", spl->ku_bit);
	for(i=0;i<32;i++) {
		serial_debug("spl->key_n[%d]:0x%08x\n",i, spl->key_n[i]);
	}
	for(i=0;i<32;i++) {
		serial_debug("spl->key_ku[%d]:0x%08x\n", i, spl->key_ku[i]);
	}

	for(i=0; i<32;i++) {
		serial_debug("spl->spl_md5_sig_kr_enc[%d]:0x%08x\n",
...
static int setup_sckeys(void *spladdr)
{
	...
	serial_debug("setup sckeys from :%x, to:%x\n", spl_sckey_addr, spltcsmptr);

	unsigned int *d = (unsigned char *)(TCSM_BANK0);
	int i;
	//for(i = 0; i<256; i++) {
		//serial_debug("d:%08x, [%d]:0x%08x\n",&d[i], i, d[i]);
	//}
	...
	//for(i=0; i<32; i++) {
		//serial_debug("spl sc key[%d]:0x%08x\n", i, spltcsmptr[i]);
	//}
...
static int start_scboot(void *spladdr, int spllen)
{
	...
	serial_debug("start scboot, spladdr :0x%x, splddrptr:0x%08x, spllen:0x%x, splbinlen:0x%x\n", spladdr,splddrptr, spllen, splbinlen);
	...
	while(splbinlen > 0) {
		...
		serial_debug("splbinlen is :%d, new round:%d, end round:%d, len:%d\n", splbinlen, newround, endround, lens);
		newround = 0;
		ret = secall(args, SC_FUNC_SCBOOT, 0, 1);
		if(ret != 0x0e000000) {
			serial_debug("sc boot test faled.ret:%08x, %08x\n", ret, *(volatile unsigned int *)MCU_TCSM_RETVAL);
		} else {
			serial_debug("wait for secall exec sucess!:%08x, %08x\n", ret, *(volatile unsigned int*)MCU_TCSM_RETVAL);
		}
		splbinlen -= SC_MAX_SIZE_PERTIME;

		for(i=0;i<8;i++) {
			serial_debug("output[%d] :0x%08x\n", i, output[i]);
		}
	}

	ret = *(volatile unsigned int *)MCU_TCSM_RETVAL;
	ret &= 0xffff;
	serial_debug("scboot ret is :%08x, should be 0\n", ret);
	if(ret == 0x00000000) {
		serial_debug("####################scboot test success!!!!##################\n");
	}
...
int test_scboot(void)
{
	...
	serial_debug("xx_splbint  1111:%08x\n", xx_splbin);
	xx_splbin = (unsigned int)xx_splbin + 512;
	serial_debug("xx_splbin 22222:%08x\n", xx_splbin);
	setup_sckeys(xx_splbin);
	serial_debug("setup sckeys to tcsm ok!\n");
