 ...
 *
 * $Id: nand_base.c,v 1.7 2007/11/28 01:19:15 zyliu Exp $
 *
...
#if 1
			if((unsigned int)(nand_read_len+from)%mtd->erasesize == 0 && oob_data[this->badblockpos]!=0xff){
			   ...
			   size_t * retlen, const u_char * buf, u_char * eccbuf, struct nand_oobinfo *oobsel)
{
	...
	*retlen = 0;
	...
	while (written < len) {
		...
		if (written_len % mtd->erasesize == 0 && nand_block_bad(mtd, page, 0)){
