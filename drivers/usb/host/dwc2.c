...
#define out_le32(port, val)     writel(val, port)
#define in_le32(port)           readl(port)
...
static int dwc_vbus_supply_init(struct udevice *dev)
{
	...
	if (ret) {
		printf("Error enabling vbus supply\n");
		return ret;
	}
...
static int dwc_vbus_supply_exit(struct udevice *dev)
{
	...
	if (priv->vbus_supply) {
		...
		if (ret) {
			printf("Error disabling vbus supply\n");
			return ret;
		}
	}
			  ...
			  int xfer_len, int *actual_len, int odd_frame, int nmulticnt)
{
	...
	//TODO: make sure phys_to_bus meaning.
	//writel(phys_to_bus((unsigned long)aligned_buffer), &hc_regs->hcdma);
	writel(virt_to_phys((unsigned long)aligned_buffer), &hc_regs->hcdma);
...
static int dwc2_init_common(struct udevice *dev, struct dwc2_priv *priv)
{
	...
	debug("Core Release: %x.%03x\n",
		 snpsid >> 12 & 0xf, snpsid & 0xfff);

	if ((snpsid & DWC2_SNPSID_DEVID_MASK) != DWC2_SNPSID_DEVID_VER_2xx &&
	    (snpsid & DWC2_SNPSID_DEVID_MASK) != DWC2_SNPSID_DEVID_VER_3xx) {
		debug("SNPSID invalid (not DWC2 OTG device): %08x\n",
			 snpsid);
		return -ENODEV;
	}
