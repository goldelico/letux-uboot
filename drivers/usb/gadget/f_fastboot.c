...
#ifdef CONFIG_NTX_SW//[
#include "../../../board/freescale/common/ntx_hwconfig.h"
		...
		if (ptn == 0) {
			...
			if (is_sparse_partition(ptn) &&
				...
				if (run_command(mmc_write, 0)) {
					...
					printf("Writing '%s' DONE!\n", ptn->name);
#ifdef CONFIG_NTX_SW//[
					if( 0==memcmp(ptn->name,"kernel",6) ||
					  	0==memcmp(ptn->name,"waveform",8) ||
					  	0==memcmp(ptn->name,"hwcfg",5) ||
					  	0==memcmp(ptn->name,"dtb",3) ||
					  	0==memcmp(ptn->name,"ntxfw",5) ||
					  	0==memcmp(ptn->name,"logo",4)	)
					{
						if(!ntx_header_update(ptn->start,download_bytes)) {
							sprintf(response, "FAIL: write %s header",ptn->name);
						}
						else {
							sprintf(response, "OKAY");
						}
...
{
	//printf("%s: adding \"%s\" to ptn %d\n",__FUNCTION__,ptn->name,pcount);
	if (pcount < MAX_PTN) {
...
static void compl_do_reset(struct usb_ep *ep, struct usb_request *req)
{
	...
	if(1==gptNtxHwCfg->m_val.bPMIC) {
		extern int RC5T619_restart(void);
...
#ifdef CONFIG_NTX_SW//[
	} else if (!strcmp_l1("hwcfg.PCB", cmd)) {
		sprintf(cTmpA, "[0] PCB=0x%02X",gptNtxHwCfg->m_val.bPCB);
		strncat(response, cTmpA, chars_left);
	} else if (!strcmp_l1("hwcfg.DisplayResolution", cmd)) {
		sprintf(cTmpA, "[31] DisplayResolution=0x%02X",gptNtxHwCfg->m_val.bDisplayResolution);
		strncat(response, cTmpA, chars_left);
	} else if (!strcmp_l1("hwcfg.RAMType", cmd)) {
		sprintf(cTmpA, "[29] RAMType=0x%02X",gptNtxHwCfg->m_val.bRamType);
		strncat(response, cTmpA, chars_left);
	} else if (!strcmp_l1("hwcfg.RamSize", cmd)) {
		sprintf(cTmpA, "[16] RamSize=0x%02X",gptNtxHwCfg->m_val.bRamSize);
		strncat(response, cTmpA, chars_left);
#endif //] CONFIG_NTX_SW
	} else {
