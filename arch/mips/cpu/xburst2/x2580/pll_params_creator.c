...
struct pll_setting
{
	...
	unsigned int vpll_freq;
#ifdef CONFIG_SYS_EPLL_FREQ
	unsigned int epll_freq;
#endif
	...
	for(n = 1; n < 64; n++) {
		for(m = 16; m < 2048; m++) {
			for(od1 = 1; od1 <8; od1++) {
				od = od0 * od1;
				fref = fin / n;
				if((fin%n) || (fref < 1) || (fref > 1200)) {
					continue;
				}
				fvco = fout * od;
				if((fref*m != fvco) || (fvco < 950) || (fvco > 3800)) {
					continue;
				}
...
int main(int argc, char *argv[])
{
	...
	struct pll_control vpll_ctrl;
	struct pll_control epll_ctrl;
	pllsetting.apll_freq = CONFIG_SYS_APLL_FREQ;
	pllsetting.mpll_freq = CONFIG_SYS_MPLL_FREQ;
	pllsetting.vpll_freq = CONFIG_SYS_VPLL_FREQ;
#ifdef CONFIG_SYS_EPLL_FREQ
	pllsetting.epll_freq = CONFIG_SYS_EPLL_FREQ;
#endif
	...
	vpll = gen_pll_regester_nfro(CONFIG_SYS_EXTAL,CONFIG_SYS_VPLL_FREQ,&vpll_ctrl);
#else
	vpll = 0;
#endif
#ifdef CONFIG_SYS_EPLL_FREQ
	...
	printf("#define VPLL_M_VALUE \t\t 0x%08x\n",vpll_ctrl.pllm);
	printf("#define VPLL_N_VALUE \t\t 0x%08x\n",vpll_ctrl.plln);
	printf("#define VPLL_OD1_VALUE \t\t 0x%08x\n",vpll_ctrl.pllod1);
	printf("#define VPLL_OD0_VALUE \t\t 0x%08x\n",vpll_ctrl.pllod0);
	printf("#define VPLL_EN_VALUE \t\t 0x%08x\n",vpll == 0? 0:1);
#ifdef CONFIG_SYS_EPLL_FREQ
