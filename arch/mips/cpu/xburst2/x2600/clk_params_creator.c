...
struct cgu cgu_clk_sel[CGU_CNT] = {
	...
	[MSC0] = {1, CPM_MSC0CDR, 30, CONFIG_DDR_SEL_PLL, {APLL, MPLL, EPLL, -1}, 29, 28, 27},
	[MSC1] = {1, CPM_MSC1CDR, 30, CONFIG_DDR_SEL_PLL, {APLL, MPLL, EPLL, -1}, 29, 28, 27},
	...
	[PWM] = {1, CPM_PWMCDR, 30, CONFIG_CPU_SEL_PLL, {APLL, MPLL, EPLL, -1}, 29, 28, 27},
...
static void gen_cgu_params(struct clk_cgu_setting *cgusetting)
{
	...
	for (id = 0; id < CGU_CNT; id++) {
		...
		if(cgu->busy) {
			/*set div max*/
			regval |= (0xf | (1 << cgu->ce) | (1 << cgu->stop));
		} else {
			/*clear ce*/
			regval &= ~(1 << cgu->ce);
		}
