...
#define AHB1CPCCR_CFG CONFIG_SYS_AHB1CPCCR_SEL
#endif
...
static void cpccr_init(void)
{
	...
	ahb1_cpccr = (cpm_inl(CPM_AHB1CDR) & 0x3)
		| (AHB1CPCCR_CFG & ~0x3)
		| (0x1 << 8);
	cpm_outl(ahb1_cpccr,CPM_AHB1CDR);
	while(cpm_inl(CPM_CPCSR) & 0xf);

	/* change sel 改变高8位 选择时钟源 */
	cpccr = (CPCCR_CFG & (0xff << 24)) | (cpm_inl(CPM_CPCCR) & ~(0xff << 24));
	cpm_outl(cpccr,CPM_CPCCR);
	ahb1_cpccr = (AHB1CPCCR_CFG & (0x3)) | (cpm_inl(CPM_AHB1CDR) & (~0x3));
	cpm_outl(ahb1_cpccr, CPM_AHB1CDR);
	//debug("cppcr 0x%x\n",cpm_inl(CPM_CPCCR));
}
