...
static void params_print(FILE *fp, int type, struct ddrc_reg *ddrc, struct ddrp_reg *ddrp)
{
	int i;
	/* DDRC registers print */
	fprintf(fp, "#define DDRC_CFG_VALUE_%d			0x%08x\n", type, ddrc->cfg.d32);
	fprintf(fp, "#define DDRC_CTRL_VALUE_%d			0x%08x\n", type, ddrc->ctrl);
	fprintf(fp, "#define DDRC_DLMR_VALUE_%d			0x%08x\n", type, ddrc->dlmr);
	fprintf(fp, "#define DDRC_DDLP_VALUE_%d			0x%08x\n", type, ddrc->ddlp);
	fprintf(fp, "#define DDRC_MMAP0_VALUE_%d		0x%08x\n", type, ddrc->mmap[0]);
	fprintf(fp, "#define DDRC_MMAP1_VALUE_%d		0x%08x\n", type, ddrc->mmap[1]);
	fprintf(fp, "#define DDRC_REFCNT_VALUE_%d		0x%08x\n", type, ddrc->refcnt);
	fprintf(fp, "#define DDRC_TIMING1_VALUE_%d		0x%08x\n", type, ddrc->timing1.d32);
	fprintf(fp, "#define DDRC_TIMING2_VALUE_%d		0x%08x\n", type, ddrc->timing2.d32);
	fprintf(fp, "#define DDRC_TIMING3_VALUE_%d		0x%08x\n", type, ddrc->timing3.d32);
	fprintf(fp, "#define DDRC_TIMING4_VALUE_%d		0x%08x\n", type, ddrc->timing4.d32);
	fprintf(fp, "#define DDRC_TIMING5_VALUE_%d		0x%08x\n", type, ddrc->timing5.d32);
	...
	fprintf(fp, "#define	DDRP_MR1_VALUE_%d		0x%08x\n", type, ddrp->mr1.d32);
	fprintf(fp, "#define	DDRP_MR2_VALUE_%d		0x%08x\n", type, ddrp->mr2.d32);
	fprintf(fp, "#define	DDRP_MR3_VALUE_%d		0x%08x\n", type, ddrp->mr3.d32);
	fprintf(fp, "#define	DDRP_PTR0_VALUE_%d		0x%08x\n", type, ddrp->ptr0.d32);
	fprintf(fp, "#define	DDRP_PTR1_VALUE_%d		0x%08x\n", type, ddrp->ptr1.d32);
	fprintf(fp, "#define	DDRP_PTR2_VALUE_%d		0x%08x\n", type, ddrp->ptr2.d32);
	fprintf(fp, "#define	DDRP_DTPR0_VALUE_%d		0x%08x\n", type, ddrp->dtpr0.d32);
	fprintf(fp, "#define	DDRP_DTPR1_VALUE_%d		0x%08x\n", type, ddrp->dtpr1.d32);
	fprintf(fp, "#define	DDRP_DTPR2_VALUE_%d		0x%08x\n", type, ddrp->dtpr2.d32);
	fprintf(fp, "#define	DDRP_PGCR_VALUE_%d		0x%08x\n", type, ddrp->pgcr);
	fprintf(fp, "#define DDRP_ODTCR_VALUE_%d                0x%08x\n", type, ddrp->odtcr.d32);
	for(i = 0;i < 4;i++){
		fprintf(fp, "#define DDRP_DX%dGCR_VALUE_%d      0x%08x\n",i,type, ddrp->dxngcrt[i].d32);
	}
	fprintf(fp, "#define DDRP_ZQNCR1_VALUE_%d               0x%08x\n", type, ddrp->zqncr1);
	fprintf(fp, "#define DDRP_IMPANDCE_ARRAY_%d             {0x%08x,0x%08x} //0-cal_value 1-req_value\n", type, ddrp->impedance[0],ddrp->impedance[1]);
	fprintf(fp, "#define DDRP_ODT_IMPANDCE_ARRAY_%d         {0x%08x,0x%08x} //0-cal_value 1-req_value\n", type, ddrp->odt_impedance[0],ddrp->odt_impedance[1]);
	fprintf(fp, "#define DDRP_RZQ_TABLE_%d  {0x%02x",, type, ddrp->rzq_table[0]);
	for(i = 1;i < sizeof(ddrp->rzq_table);i++){
		fprintf(fp, ",0x%02x",ddrp->rzq_table[i]);
	}
	fprintf(fp, "}\n");
#else
	fprintf(fp, "#define DDRP_MEMCFG_VALUE_%d		0x%08x\n", type, ddrp->memcfg.d32);
	fprintf(fp, "#define DDRP_CL_VALUE_%d			0x%08x\n", type, ddrp->cl);
	fprintf(fp, "#define DDRP_CWL_VALUE_%d			0x%08x\n", type, ddrp->cwl);
#endif
}
...
static void ddr_mr_print(FILE *fp, int type, struct ddr_params *p)
{
	...
	fprintf(fp, "#define	DDR_MR1_VALUE_%d			0x%08x\n", type, p->mr1.d32);
	fprintf(fp, "#define	DDR_MR2_VALUE_%d			0x%08x\n", type, p->mr2.d32);
	fprintf(fp, "#define	DDR_MR3_VALUE_%d			0x%08x\n", type, p->mr3.d32);
	fprintf(fp, "#define	DDR_MR10_VALUE_%d			0x%08x\n", type, p->mr10.d32);
	fprintf(fp, "#define	DDR_MR63_VALUE_%d			0x%08x\n", type, p->mr63.d32);
}
#endif

static void sdram_size_print(FILE *fp, int type, struct ddr_params *p)
{
	fprintf(fp, "#define	DDR_CHIP_0_SIZE_%d			%u\n", type, p->size.chip0);
	fprintf(fp, "#define	DDR_CHIP_1_SIZE_%d			%u\n", type, p->size.chip1);
}
 __attribute__((unused)) static unsigned int frandom(int max)
{
	return max;
}
static void mem_remap_print(FILE *fp, int type, struct ddr_params *p)
{
	int address_bits;
	int swap_bits;
	int bank_bits;
...
#define swap_bytes(buf,b1,b2) do{		\
	...
	/* 	swap_bytes(s,startA + i,startA + sw); */
	/* } */
	fprintf(fp, "#define REMMAP_ARRAY_%d {\\\n", type);
...
static void file_head_print(FILE *fp, int type)
{
	fprintf(fp, "/*\n");
	...
	fprintf(fp, "#ifndef __DDR_REG_VALUES_%d_H__\n", type);
	fprintf(fp, "#define __DDR_REG_VALUES_%d_H__\n\n", type);
}

static void file_end_print(FILE *fp, int type)
{
	fprintf(fp, "\n#endif /* __DDR_REG_VALUES_%d_H__ */\n", type);
}


#define MAX_DDR_TYPE	5
static struct ddr_creator_ops *g_ddr_creator[4] = {NULL, NULL, NULL, NULL};

static int ops_count = 0;
void register_ddr_creator(struct ddr_creator_ops *ops)
{
	if(ops_count < MAX_DDR_TYPE) {
		g_ddr_creator[ops_count] = ops;
		ops_count ++;
	} else {
		out_error("Error: DDR CREATOR too many ops, type %d, ops_count %d\n", ops->type, ops_count);
	}
}


void create_one_params(struct ddr_creator_ops *ops, char *file)
{
	struct ddrc_reg ddrc;
	struct ddrp_reg ddrp;
	struct ddr_params ddr_params;
	struct ddr_creator_ops *p_ddr_creator = ops;
	FILE *fp;
	...
	mem_remap_print(fp, p_ddr_creator->type, &ddr_params);
	file_end_print(fp, p_ddr_creator->type);
...
int main(int argc, char *argv[])
{
	...
	create_one_params(g_ddr_creator[i], argv[1]);
