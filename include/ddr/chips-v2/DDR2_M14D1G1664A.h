 ...
 */
	...
	c->DDR_tRP = DDR__ps(16000); /* tRP: PRECHARGE command period to the same bank */
	c->DDR_tRCD = DDR__ps(16000);  /* ACTIVE to READ or WRITE command period to the same bank. */
//	c->DDR_tRC		(DDR_tRAS + DDR_tRP)  /* ACTIVE to ACTIVE command period to the same bank.*/
	c->DDR_tRC = DDR__ps(56250);  /* ACTIVE to ACTIVE command period to the same bank.*/
 ...
 */
	c->DDR_tRFC = DDR__ps(127500); /* ns,  AUTO-REFRESH command period. */
...
/*	c->DDR_WL		(DDR_CL - 1)[>(DDR_tRL - 1)        DDR2: Write Latency = tAL + tCL - 1<]*/
	...
	c->DDR_tRDLAT = DDR__tck(7); //(c->DDR_tRL - DDR__tck(2));
