...
static inline void DDR2_W971GV6NG_init(void *data)
{
	...
	c->DDR_BANK8 = 1;/* Banks each chip: 0-4bank, 1-8bank */
 ...
 */
	...
	c->DDR_tRTP = DDR__ps(7500);   /* 7.5ns READ to PRECHARGE command period. */
...
//	c->DDR_tRC		(DDR_tRAS + DDR_tRP)  /* ACTIVE to ACTIVE command period to the same bank.*/
	...
	c->DDR_tRRD = DDR__ns(10);  /* ACTIVE bank A to ACTIVE bank B command period. */
 ...
 */
	c->DDR_tRFC = DDR__ns(128); /* ns,  AUTO-REFRESH command period. */
...
/*	c->DDR_tRDLAT	(DDR_RL - DDR__tck(2))*/
/*	c->DDR_tWDLAT	DDR__tck(3)*//*(DDR_tWL - 1)*/
	c->DDR_tXARD = DDR__tck(3);     /* DDR2 only: Exit active power down to read command , tCK*/
	c->DDR_tXARDS = DDR__tck(10 - c->DDR_AL);/* DDR2 only: Exit active power down to read command (slow exit/low power mode), tCK */
