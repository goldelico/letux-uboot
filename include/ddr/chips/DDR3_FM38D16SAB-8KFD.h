...
#define DDR_ROW1    0  /* ROW : 12 to 18 row address ,1G only 512MB*/
#define DDR_COL     10  /* COL :  8 to 14 column address */
#define DDR_COL1    0  /* COL :  8 to 14 column address */
#define DDR_BANK8   1 	/* Banks each chip: 0-4bank, 1-8bank */
#define DDR_CL      5  /* CAS latency: 7 is much more stable than 6 on mensa board */

//#define DDR_tCWL   (DDR_CL - 1)	/* DDR3 only: CAS Write Latency, 5 to 8 */

#define DDR_tCWL   (5)	/* DDR3 only: CAS Write Latency, 5 to 8 */

/*
 * DDR3 controller timing1 register
 */
#define DDR_tRAS DDR__ns(38)  /* tRAS: ACTIVE to PRECHARGE command period to the same bank. ns*/
#define DDR_tRP  DDR__ps(14000)  /* tRP: PRECHARGE command period to the same bank. ns*/
#define DDR_tRCD DDR__ps(14000)  /* ACTIVE to READ or WRITE command period to the same bank. ns*/
#define DDR_tRC  DDR__ps(51000)  /* ACTIVE to ACTIVE command period to the same bank. ns*/
...
#define DDR_tFAW   DDR__ns(27)	/* Four bank activate period, DDR3 - tCK */
#define DDR_tCKE   	DDR_SELECT_MAX__tCK_ps(3, 5625)	/* CKE minimum pulse width, DDR3 spec no, tCK */
...
//#define DDR_tXS    	DDR_SELECT_MAX__tCK_ps(5, 120000) /* DDR3 only: EXit self-refresh to command not requiring a locked DLL, ns*/
#define DDR_tXS    	DDR__tck(5) /* DDR3 only: EXit self-refresh to command not requiring a locked DLL, ns*/
